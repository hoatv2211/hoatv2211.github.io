...
assert.strictEqual(current.length, 22, "runtime compatibility set must contain 22 active records");
...
for (const project of current.filter((candidate) => ["muloren", "jx1", "dalgona", "idleCyber", "nekoverse"].includes(candidate.id))) {
  assert.match(project.detailUrl || "", /^projects\/[a-z0-9-]+\/$/, `${project.id} must expose its canonical detail route`);
}
console.log("Portfolio compatibility passed for 22 active records.");
